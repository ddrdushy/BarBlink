...
export default function CreateStoryScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data = await venueGet<{ items: VenueOption[] }>(
          `/venues?country=${profile.country}&limit=50`,
        );
  ...
  const handlePost = async () => {
    ...
    setError('');
    try {
      const mediaUrl = await uploadImage(imageUri, token);
      const body: Record<string, unknown> = {
        mediaUrl,
        mediaType: 'photo',
      };
      if (venue) body.venueId = venue.id;
      await socialPost('/stories', body, token);
      router.back();
    } catch (e: any) {
    ...
    <Screen>
      ...
      {imageUri ? (
        ...
        <View style={styles.previewPlaceholder}>
          ...
          <Text style={styles.placeholderText}>Pick a photo for your story</Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  placeholderText: { color: colors.inkFaint, fontSize: 14 },
  venuePicker: {
    backgroundColor: colors.bgSurface,
    borderRadius: 12,
    borderWidth: 1,
