...
const Ctx = createContext<DjAuth>({ token: null, setToken: () => {}, logout: () => {} });
...
export function DjAuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('bbk_dj_token');
  ...
  const setToken = (t: string | null) => {
    setTokenState(t);
    if (t) localStorage.setItem('bbk_dj_token', t);
    else localStorage.removeItem('bbk_dj_token');
  };
